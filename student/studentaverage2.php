...
$dr = mysqli_query($cn, "select * from generation 
	where year = '" . $_SESSION['year'] . "' AND semester = '" . $_SESSION['semester'] . "' AND classe='" . $_SESSION['classe'] . "'");
...
if ($resultat[4] == 0) {
	...
	<form action="" method="post">
		...
		<?php
		$dr = mysqli_query($cn, "SELECT name, firstname, birthdate FROM student 
WHERE code='" . addslashes($_SESSION['student']) . "'");
		...
		// requete sql
		$query = "SELECT coursename, average, niveau, niveauG, course, classe 
FROM averagetotal 

WHERE student ='" . addslashes($_SESSION['student']) . "'
AND semester='" . addslashes($_SESSION['semester']) . "' 
AND year='" . addslashes($_SESSION['year']) . "' 
AND classe='" . addslashes($_SESSION['classe']) . "'
ORDER BY course ASC";
		...
		<br><br><br><br><br>
		<div id="global2">
			<center><input name="b3" class="btn btn-success" type="submit" class="btn btn-success" value="Zurück" id="b3">
			</center>
		</div>
		<br>
		<div class="container">
