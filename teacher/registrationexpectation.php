...
<section>
	<div class="col-lg-12 text-center">
		<h2>Anmeldungen pro Warteliste</h2>
	</div>
	<form id="form1" name="form1" method="post">
		...
		<table width="300" border="0" align="center">
			...
			<tr>
				<td>gewünschte Klasse</td>
				<td><select name="c10" style="width:170px;">
						<option value="">Wählen Sie aus</option>
						<?php
						$requete = 'SELECT code FROM `classerequired`';
						$result = mysqli_query($cn, $requete);
						while ($ligne = mysqli_fetch_array($result)) {
							$selectionnee = ($ligne[0] == $_POST["c10"]) ? " SELECTED " : "";
		...
		if (isset($_POST['b2'])) {
			if (addslashes($_POST['c1']) == '' or addslashes($_POST['c2']) == '' or addslashes($_POST['c10']) == '') {
				...
				<table width="80%" border="2" align="center" class="table-style-two">
					<tbody>
						...
						$dr = mysqli_query($cn, "SELECT r.code, s.name, s.firstname, s.adresse, s.cp, s.city,  s.email, r.semester, r.classe, r.year, s.code FROM registration r, student s WHERE s.code = r.student and r.semester='" . addslashes($_POST['c2']) . "' and r.year='" . addslashes($_POST['c1']) . "' and r.classerequired='" . addslashes($_POST['c10']) . "' and r.classe=''");
				...
				<table width="300" border="0" align="center">
					<tr>
						<td><b>ausgewählte Schüler einschreiben in:</b></td>
					</tr>
					<tr>
						<td>Jahr</td>
						<td>
			...
			if ($_POST['c3'] == '') {
				...
				if (!empty($_POST['chck'])) {
					foreach ($_POST['chck'] as $value) {
						$requete = "UPDATE registration SET classe='" . addslashes($_POST['c3']) . "' WHERE year='" . addslashes($_POST['c1']) . "' AND semester='" . addslashes($_POST['c2']) . "' AND classerequired='" . addslashes($_POST['c10']) . "' AND student='" . addslashes($value) . "'";
						$result = mysqli_query($cn, $requete);
					}
				} else {
					echo "<center><img src='../img/error.png' alt='reussi' WIDTH=25 HEIGHT=25 />&nbsp;&nbsp;<b><font color='red'> Alle Felder müssen ausgefüllt sein. </center></b></font><br>";
